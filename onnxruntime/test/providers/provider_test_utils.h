...
class OpTester {
 public:
  ...
  void AddSparseCooOutput(const char* name, const std::vector<int64_t>& dims,
                          ...
                          const std::vector<int64_t>& expected_indices, const CheckParams& check_params) {
    auto ml_type = DataTypeImpl::GetType<T>();
 ...
 private:
  ...
  void AddSparseTensorData(std::vector<Data>& data, const char* name, NodeArg node_arg,
